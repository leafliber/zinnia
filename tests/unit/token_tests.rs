...
mod token_generation {
    ...
    fn test_generate_device_api_key_live() {
        ...
        assert!(
            result.token.starts_with("zn_live_"),
            "令牌应以 zn_live_ 开头"
        );
        assert!(
            result.display_prefix.starts_with("zn_live_"),
            "显示前缀应以 zn_live_ 开头"
        );
        assert!(
            result.display_prefix.ends_with("..."),
            "显示前缀应以 ... 结尾"
        );
...
mod token_verification {
    ...
    fn test_verify_token_success() {
        ...
        assert!(
            verify_token(&result.token, &result.hash).unwrap(),
            "正确的令牌应验证通过"
        );
    }
    ...
    fn test_verify_token_wrong_token() {
        ...
        assert!(
            !verify_token("wrong_token", &result.hash).unwrap(),
            "错误的令牌应验证失败"
        );
    }
    ...
    fn test_verify_token_modified_token() {
        ...
        assert!(
            !verify_token(&modified, &result.hash).unwrap(),
            "篡改的令牌应验证失败"
        );
    }
...
mod token_type_detection {
    ...
    fn test_token_type_from_token() {
        assert_eq!(
            TokenType::from_token("zn_live_abc123"),
            Some(TokenType::DeviceApiKeyLive)
        );
        assert_eq!(
            TokenType::from_token("zn_test_abc123"),
            Some(TokenType::DeviceApiKeyTest)
        );
        assert_eq!(
            TokenType::from_token("zn_dat_abc123"),
...
mod search_prefix_extraction {
    ...
    fn test_extract_search_prefix() {
        ...
        assert_eq!(
            search_prefix, result.display_prefix,
            "搜索前缀应与显示前缀相同"
        );
    }
